...
def autopad(k, p=None):  # kernel, padding 当不指定p时，Conv2d 的pad 值是让 k//2(除法向下取整)计算得到的(aotopad函数)
    # Pad to 'same'
    if p is None:
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers):
        ...
        self.c2 = c2  # 输出的channel

    def forward(self, x):
        if self.conv is not None:  # embedding
            x = self.conv(x)
        b, _, w, h = x.shape
        p = x.flatten(2).unsqueeze(0).transpose(0, 3).squeeze(3)  # flatten(dim): 从第dim维开始flatten
        """
        p = x.flatten(2)
        p = p.unsqueeze(0)
        p = p.transpose(0,3)
